...
\section{Evolutionary algorithms}
	...
	Because they are not tied to a fixed type of problem, they find use in fields as disparate as computer science, medicine, economics or cryptography, among many others. In the business at hand, they will serve as neural network optimizers, in addition to feature selectors in a preliminar step.
...
\newpage
	...
	\begin{itemize}
		...
		\item
		Population: a list of candidate solutions (called \textit{individuals}) to a problem.

		\item
		Selection process: every genetic algorithm has one or more fitness functions which evaluate the quality of an individual. On that basis, we can choose some \textit{parents} to help us find better individuals.
