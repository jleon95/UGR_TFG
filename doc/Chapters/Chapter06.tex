...
\section{Structure optimization}
	...
	Notice that the size of the offspring population is now determined by the sum of the two probabilities. The mutation will typically occur more often, thus making the algorithm resemble a random search with guiding criteria.

	Also, in order to improve the exploration, we could have different mutation operators and alternate between them at random.
