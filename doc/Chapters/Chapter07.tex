...
\section{Learning optimization}\label{sec:res_lo}
	...
	At the time of the writing, we only had time for two experiments with each test subject (around 8 hours each). In Figure \ref{gfx:learning_evolution} we display the evolution of one of them for each subject to show that the progression is indeed stable. Table \ref{table:average_best_learning} contains the average peak performances; it is plausible (and expected) that the peak results improve as we rerun the algorithm from different starting points.
...
\newpage
	...
	\begin{table}[h]
        ...
        \begin{tabular}{| >{\centering\arraybackslash}m{0.7in} | >{\centering\arraybackslash}m{1.1in} | >{\centering\arraybackslash}m{1.1in} |}
            ...
            \hline
            \cellcolor{RoyalBlue}\textbf{107} & $ \pm $ & $0.08426$ \\
            \hline
            \cellcolor{RoyalBlue}\textbf{110} & $ \pm $ & $0.12636$ \\
            \hline
    ...
    \begin{itemize}
    	...
    	\item
    	Subject 107: $46$ features, $2$ hidden layers ($89$ units, $102$ units), $ $ learning rate, $ $ dropout, ELU activation function.

    	\item
    	Subject 110: $50$ features, $1$ hidden layer ($34$ units), $ $ learning rate, $ $ dropout, ELU activation function.
...
\section{Efficiency study}
	...
	\subsection{Feature selection: Logistic Regression and \acs{SVM}s}
        ...
        \caption[Average running times: Logistic Regression versus \acs{SVM}]{Comparison of average running times for Logistic Regression and \acs{SVM}. Run in Intel® Xeon® E5-2620 v4 @ 2.10GHz.}\label{table:logreg_svm_speed}

    \end{table}

	\subsection{Feature selection: sequential and parallel}

	\subsection{Structure search: simplicity and cross-validation}
