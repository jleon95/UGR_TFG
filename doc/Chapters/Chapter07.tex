...
\newpage
    ...
    Again, the test shows that there is a relevant accuracy boost when using \acs{SVM} instead of Logistic Regression. It is also true that this change has taken a toll on the running time of the algorithm. 

    Nevertheless---and following the same logic as when we increased population sizes and number of generations---, the improvements in quality of the solutions are crucial at this level. Since we can still run a couple dozen experiments a day, we can safely sacrifice efficiency in favor of further gains.

	We have finally completed our set of feature selection enhancements. Although by now it should go without saying that feature selection is essential in our problem, Table \ref{table:feature_selection_comparison} puts our best results (in terms of Kappa loss) against classifiers using all 3600 features.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{| >{\centering\arraybackslash}m{0.7in} | >{\centering\arraybackslash}m{0.7in} | >{\centering\arraybackslash}m{0.7in} | >{\centering\arraybackslash}m{0.7in} | >{\centering\arraybackslash}m{0.7in} |}
            ...
            \hline
            \cellcolor{RoyalBlue}\textbf{104} & $0.042246$ & $0.033780$ & $0.279954$ & $0.305215$ \\
            \hline
            \cellcolor{RoyalBlue}\textbf{107} & $0.101059$ & $0.084232$ & $0.327885$ & $0.328009$ \\
            \hline
            \cellcolor{RoyalBlue}\textbf{110} & $0.117981$ & $0.109533$ & $0.404468$ & $0.404545$ \\
            \hline

        \end{tabular}

        \caption{Kappa loss of feature selection versus no feature selection.}\label{table:feature_selection_comparison}
